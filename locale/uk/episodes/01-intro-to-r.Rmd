...
Ви можете отримати вихід з R, просто ввівши математику в консолі:
...
Все, що існує в `R` - це об'єкти: від простих чисельних значень до рядків, до складніших об'єктів, таких як вектори, матриці та списки. Навіть вирази й функції є об'єктами в `R`.

Однак, щоб робити корисні та цікаві речі, нам потрібно назвати об'єкти. Для цього нам потрібно дати _ім'я_, за яким слідує оператор присвоєння `<-`, і _об'єкт_, який ми хочемо назвати:
...
для завдань, але не в кожному контексті. Через
[незначні відмінності] (https://blog.revolutionanalytics.com/2008/12/use-equals-or-arrow-for-assignment.html)
у синтаксисі, рекомендується завжди використовувати `<-` для завдань. Більше
...
У RStudio ввівши <0>Alt</0> + <0>\ -</0> натискайте <0>Alt</0>
одночасно з клавішею <0>\ -</0>  запише `<- `одним натисканням клавіші на ПК, при цьому набираючи <0>Option
</0> + <0>\ -</0> натискайте <0>Варіант</0> одночасно з клавішею <0>\ -</0>  робить те ж саме в Mac.
...
уникати крапок (`.`) у назві об'єкта, як у `my.dataset`. There are many
